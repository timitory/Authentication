<template>
    <div class="header w-full py-4 top-0 justify-between static bg-gray-800">
      <div class="flex justify-between items-center w-full px-4">
        ...
        <div class="hidden md:flex items-center space-x-5 text-white">
          <form class="outline-0 border-0">
            <select name="language" id="language" class="text-white outline-0 border-0 bg-gray-800">
          ...
          <div v-else>
            <RouterLink to="/signup" class="px-2">SignUp</RouterLink>
      ...
      <div v-if="isMobileMenuOpen" class="md:hidden">
        <div class="flex flex-col items-center px-4 py-2 space-y-2 text-white">
          <div class="relative flex items-center bg-white rounded-md w-full">
            <svg 
              class="absolute left-0 transform -translate-y-1/2 h-5 w-5 text-gray-400 mt-4 ml-2 cursor-pointer" 
              xmlns="http://www.w3.org/2000/svg" 
              fill="none" 
            ...
            <input 
              type="text" 
              placeholder="Search for our products..." 
              class="py-3 border-0 outline-0 rounded-md px-6 ml-4 w-full text-sm text-gray-800"
            >
          </div>
          <form class="outline-0 border-0">
            <select name="language" id="language" class="text-white outline-0 border-0 w-full bg-gray-800">
              <option value="english">English</option>
              <option value="portuguese">Portuguese</option>
              <option value="spanish">Spanish</option>
              <option value="french">French</option>
          ...
          <RouterLink to="/home" class="text-center">Home</RouterLink>
          <RouterLink to="/" class="w-full text-center">PaddyBlogs</RouterLink>
          <div v-if="isAuthenticated" class="text-center">
            <button @click="OnLogout" class="w-full text-left">LogOut</button>
          </div>
          <div v-else class="text-center">
            <RouterLink to="/signup" class="w-full text-left pr-2">SignUp</RouterLink>
            <RouterLink to="/login" class="w-full text-left">Login</RouterLink>
          </div>
          <div class="relative text-center">
            <RouterLink to="/cart" class="w-full text-center flex items-center justify-center">
              <img src="../assets/cartimage.png" alt="cartimage" class="w-10 h-10">
              <span v-if="cartItemCount > 0" class="absolute top-0 right-0 inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-red-100 bg-red-600 rounded-full">{{ cartItemCount }}</span>
            </RouterLink>
          </div>
